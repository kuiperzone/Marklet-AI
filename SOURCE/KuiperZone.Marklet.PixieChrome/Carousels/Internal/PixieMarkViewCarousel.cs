...
internal sealed class PixieMarkViewCarousel : PixieCarousel
{
    public PixieMarkViewCarousel()
    {
        Title = nameof(PixieMarkView);
        Symbol = Symbols.Markdown;

        // SWITCH
        var group = CreateGroup(nameof(PixieMarkView));

        var control = CreateControl<PixieMarkView>(group);
        control.Subject.Content = GetAssistantMessage();
        control.Subject.IsChromeStyled = true;
        control.Footer = "ASSISTANT STRETCH";

        control = CreateControl<PixieMarkView>(group);
