...
/// </summary>
internal sealed class PixieProgressCarousel : PixieCarousel
{
    public PixieProgressCarousel()
    {
        Title = nameof(PixieProgress);
        Symbol = Symbols.Chronic;

        var group = CreateGroup(nameof(PixieProgress));

        var control = CreateControl<PixieProgress>(group, "");
        AddAccoutrements(control);
        control.Subject.Value = 50;
        control.Subject.ShowProgressText = true;

        control = CreateControl<PixieProgress>(group, "");
        control.Header = null;
        control.Footer = null;
        control.Subject.Value = 50;
        ...
        control = CreateControl<PixieProgress>(group, false);
        control.Subject.Value = 50;
    }
}