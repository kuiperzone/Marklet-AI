...
/// Base class.
/// </summary>
internal abstract class PixieCarousel : CarouselPage
{
    protected PixieGroup CreateGroup(string? title = null)
    {
        var g = new PixieGroup();
        g.CollapseTitle = title;
        g.IsCollapsible = title != null;
        g.CollapseFooter = $"{nameof(PixieGroup)}.{nameof(PixieGroup.CollapseFooter)}";
        Children.Add(g);
        return g;
    }

    protected static T CreateControl<T>(PixieGroup? group, string? title = null)
        where T : PixieControl, new()
    {
        return CreateControl<T>(group, true, title);
    }

    protected static T CreateControl<T>(PixieGroup? group, bool enabled, string? title = null)
        where T : PixieControl, new()
    {
        var c = new T();
