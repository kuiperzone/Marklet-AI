...
/// ensure uniqueness of values with same timestamp value. The "Z" first letter in the name originates from an earlier
/// project that was to be called "ZEN".
/// </remarks>
public readonly struct Zuid : IComparable<Zuid>, IEquatable<Zuid>
{
    ...
    /// <summary>
    /// Generates a new timestamped value from the system clock but offset from the current time.
    /// </summary>
    /// <exception cref="ArgumentOutOfRangeException">Clock out of range</exception>
    public static Zuid New(TimeSpan offset)
    {
        return new(DateTime.UtcNow + offset);
    }
