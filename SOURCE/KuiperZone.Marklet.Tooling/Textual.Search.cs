...
{
    /// <summary>
    /// Returns the first index of "keyword" string from the given "startIndex" according to flags, or -1 if not found.
    /// </summary>
    /// <remarks>
    ...
    public static int Search(this string src, string? keyword, int startIndex, SearchFlags flags, int scanLimit = int.MaxValue)
    {
        ...
        if (string.IsNullOrEmpty(keyword) || keyword.Length > Math.Min(src.Length, scanLimit))
        {
            return -1;
        }
        ...
        while ((startIndex = src.IndexOf(keyword, startIndex, scanLimit, comp)) > -1)
        {
            if (!flags.HasFlag(SearchFlags.Word) ||
                (IsWordBoundaryAt(src, startIndex - 1) &&
                IsWordBoundaryAt(src, startIndex + keyword.Length)))
            {
                return startIndex;
            }
    ...
    /// <summary>
    /// Returns the first index of "keyword" string according to "flags", or -1 if not found.
    /// </summary>
    /// <remarks>
    /// This is an extended "IndexOf" routine. The "scanLimit" givens the maximum number of positions to examine from
    /// "startIndex". It may exceed the length of "src", but cannot be negative. The "scanLimit" may be used to limit
    ...
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException">startIndex, or negative scanLimit</exception>
    public static int Search(this string src, string? keyword, SearchFlags flags, int scanLimit = int.MaxValue)
    {
        return Search(src, keyword, 0, flags, scanLimit);
    }

    /// <summary>
    /// Finds first instance of "keyword" in "src" and returns a "snippet" on success.
    /// </summary>
    /// <remarks>
    /// The result is may include short a text prior to the "keyword" occurrance and is null if no match is found.
    ...
    public static string? PrettySearch(this string src, string? keyword, int maxLength, SearchFlags flags, int scanLimit = int.MaxValue)
    {
        var index = Search(src, keyword, 0, flags, scanLimit);
